...
console.log(`${which}: ${wasmBytes.byteLength} bytes (${(wasmBytes.byteLength / 1024).toFixed(2)} KB)`);
...
for (const [name, val] of Object.entries(instance.exports)) {
  const kind = typeof val === "function" ? "func" : val instanceof WebAssembly.Memory ? "memory" : typeof val;
