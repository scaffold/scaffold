...
- [ ] Claim/ref links never notify on ingestion. `BlockIngestor.ingest` emits `LinkAnchor`/`LinkAnchoringNode`/`LinkAggregate`/`LinkAggregatingNode` but nothing for the block's own claims or refs, so a producer's listeners never see `LinkClaimingNode` when someone claims its output. The only path that fires those actions is the BlockRef-relink loop in `deserialize` (behind a `setTimeout`). `FetchService.fetch` works around it by rescanning `payload.claims` on every `BlockStore.onIngest` -- and its `LinkClaimingNode` version sits commented out. `BlockActionType` also has no ref-side equivalents (`LinkRef`/`LinkReffingNode`), which `Block.refs` now needs
- [ ] `tests/Placement2.test.ts` is stale and has 13 type errors: it targets the pre-generic `PlacementModule` (`getCanonicalAggregator`, `logger()`, `result.anchor` rather than `result.anchorChain`, and a `PlacementRequest` with no `genesis`). Retarget it at `PlacementModule<NodeType>` or drop it
