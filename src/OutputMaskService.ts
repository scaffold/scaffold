...
type MaskDetail = Pick<FrontierTreeDetail, 'frontierVoteOutputMask' | 'frontierVoteOutputCount' | 'subtreeOutputCount'>;
...
export class OutputMaskService {
  ...
  public test(block: BlockFact, outputIndex: number) {
    ...
    return (block.frontierDetail.frontierVoteOutputMask[byte] >>> (outputIndex & 7)) & 1;
  }
  ...
  public mapChildOutput(block: BlockFact, childIdx: number, outputIndex: number) {
  ...
  private remap(block: BlockFact, src: Uint8Array) {
    const out = new Uint8Array(block.frontierDetail.frontierVoteOutputMask.byteLength);
    bitScatter(out, block.frontierDetail.frontierVoteOutputMask, src, block.frontierDetail.subtreeOutputCount);
    return out;
  }
  // remap(a, remap(b, c))
    ...
    if (path !== undefined) {
      ...
      for (const next of path) {
        if (next === prev.frontierVoteBlock) {
          detail.frontierVoteOutputMask = this.remap(next, detail.frontierVoteOutputMask);
        } else {
    ...
    if (path !== undefined) {
      ...
      for (const next of path) {
        if (next.frontierVoteBlock === prev) {
          assert(next.frontierDetail.frontierVoteOutputCount === detail.frontierVoteOutputCount);
          assert(next.frontierDetail.frontierVoteOutputMask.byteLength === detail.frontierVoteOutputMask.byteLength);

          for (let i = 0; i < next.frontierDetail.frontierVoteOutputMask.byteLength; i++) {
            if (next.frontierDetail.frontierVoteOutputMask[i] & detail.frontierVoteOutputMask[i]) {
          ...
          // ???
          detail.frontierVoteOutputMask = this.remap(detail.frontierVoteOutputMask, next.frontierDetail.frontierVoteOutputMask);
        } else {
