  ...
  listActiveGames(): ActiveGame[] {
    ...
    for (const block of ctx.store.values()) {
      for (let i = 0; i < block.outputs.length; i++) {
        ...
        // Atomic claim invariant (Node graph): an output is unspent iff
        // no canonical Node has a claim pointing here. Drafts holding
        // reservations count as spends just like blocks do.
