...
| [`src/core/BlockVerificationModule.ts`](../../src/core/BlockVerificationModule.ts) | Per-block orchestrator: enumerates `claims`, dispatches per-verifier verification, fail-fast aggregation, defers on unresolved claims. Layer 3 of the verification flow. |
