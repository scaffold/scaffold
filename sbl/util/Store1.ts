...
export default class Store1<Atom> {
  ...
  public innerJoin<Atom2>(rhs: Store1<Atom2>) {
    const res = new Store1<[Atom, Atom2]>();
  ...
  public leftJoin<Atom2>(rhs: Store1<Atom2>) {
    const res = new Store1<[Atom, Atom2 | null]>();
    this.entries.forEach((e1, key) => {
      const e2 = rhs.entries.get(key);
      const hash = Hash.fromPrimitive(key);
      if (e2) {
  ...
  public rightJoin<Atom2>(rhs: Store1<Atom2>) {
    const res = new Store1<[Atom | null, Atom2]>();
...
// Propagate value back to each store1. Value is only meaningful per-store1. If I had dropped this element when I got it, what value would I have lost?
// Can do ML on each store1 individually to understand what to keep and what to drop.
