...
import WorkerDriverService, { ExecutorDriver } from './WorkerDriverService.ts';
...
export interface ComputationDriver {
...
export default class WorkerLauncherService {
  ...
  private makeVerificationDriver(
  ...
  ): ComputationDriver & { getVerificationResult(): string } {
    ...
    return {
      ...
      getBody: () => block.body,
      requireBody: (data) => {
        if (!executorDriver.isRunning()) {
          return;
        }
        if (!arrEquals(block.body, data)) {
          isValid = false;
          executorDriver.cancel(INTERRUPT_FLAG);
        }
      },
  ...
  private makeGenerationDriver(
    verifier: Verifier,
    executorDriver: ExecutorDriver,
  ): ComputationDriver & { getBlockSpec(): BlockSpec } {
    ...
    return {
      ...
      request: (verifier) =>
        new Promise((reply) => {
          ...
          executorDriver.onCleanup(release);
        }),
