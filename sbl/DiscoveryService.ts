...
export default class DiscoveryService {
  ...
  private makeNode(bridge: Connection, toNodeHash: Hash) {
    const onListen = (spec: ConnectionSpec) => {
      if (!node.conn) {}
    };
    const onNewConn = (provider: ConnectionProvider) =>
      node.conn = this.ctx.get(ConnectionService).makeConnection(provider);
    const connector = this.ctx.config.networkProvider.create(
      onListen,
      onNewConn,
    );
