...
import { FetchService } from '../src/node/FetchService.ts';
...
function harness(): Harness {
  ...
  const publish = (outputs: Output[], claims: bigint[]): Block => {
    ...
    const raw = ctx.get(AtomSerializerService).serialize(AtomType.Block, payload);
...
// src/node/FetchService.ts:50 only subscribes to future ingestions, so a fetch for a
...
// src/node/FetchService.ts:29-32 creates and builds the draft before looking at
...
// src/node/FetchService.ts:63 hands input.signal to BlockStore.onIngest only, so the
