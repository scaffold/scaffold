...
import {
  ...
  AtomSerializerService,
  headerSize,
} from '../src/core/AtomSerializer.ts';
import { Ingestor } from '../src/core/Ingestor.ts';
import { Atom, AtomBase, AtomSource, AtomType, BlockPayload, Signal } from '../src/core/types.ts';
...
const makeService = (selfPrivateKey?: Uint8Array) =>
  new AtomSerializerService(makeTestContext({ selfPrivateKey }));
...
/** Second instantiation of the abstract module, as `GenesisSerializer` in src/genesis.ts is. */
class TestSerializer extends AtomSerializerModule {
...
Deno.test('a genesis block written by GenesisSerializer is readable by AtomSerializerService', () => {
