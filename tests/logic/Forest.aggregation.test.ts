...
Deno.test('a lone node is its own only aggregator', () => {
  ...
  assertEquals(names(new ForestBase().aggregators(x)), ['X']);
});
...
Deno.test('aggregators walks the aggregation chain transitively', () => {
  ...
  assertEquals(names(new ForestBase().aggregators(x)), ['X', 'Z', 'W']);
});
...
Deno.test('competing aggregators of the same block are all returned', () => {
  ...
  assertEquals(names(new ForestBase().aggregators(x)), ['X', 'Z1', 'Z2']);
});
...
Deno.test('a diamond yields each aggregator once', () => {
  ...
  const result = new ForestBase().aggregators(x);
...
Deno.test('a lone node has exactly one aggregation chain', () => {
  ...
  const chains: string[][] = [];
  for (const chain of new ForestBase().aggregationChains(x)) chains.push(names(chain));
...
Deno.test('a collected chain is the same mutated array -- callers must copy', () => {
  ...
  const collected = [...new ForestBase().aggregationChains(x)];
...
Deno.test('the shared base array is restored after a full walk', () => {
  ...
  const base = [node('S')];
  const chains: string[][] = [];
  for (const chain of new ForestBase().aggregationChains(x, base)) chains.push(names(chain));
...
Deno.test('abandoning the walk early leaves the base array mutated', () => {
  ...
  const base: AggNode[] = [];
  for (const _ of new ForestBase().aggregationChains(x, base)) break;
