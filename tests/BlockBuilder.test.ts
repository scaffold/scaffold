...
import { BlockBuilderModule, BlockBuilderService } from '../src/core/BlockBuilderModule2.ts';
import { BlockStore } from '../src/core/BlockStore.ts';
import { AnchorChainNode } from '../src/core/ClaimIndexService.ts';
import { PlacementNode, PlacementRequest, PlacementResult } from '../src/core/PlacementService2.ts';
import {
...
import { AggregatorNodeBase } from '../src/core/ForestService.ts';
...
class StubBuilder extends BlockBuilderModule {
...
const okPayload = (result: ReturnType<BlockBuilderModule['build']>) => {
...
const ingestBlock = (ctx: Context, attrs: Partial<BlockPayload>): Block =>
  ctx.get(BlockStore).ingest({
    ...
    receivedAt: 0,
    raw: ctx.get(AtomSerializerService).serialize(AtomType.Block, {
      anchor: ZERO_HASH,
      chain: [],
      aggregates: [],
      claims: [],
...
Deno.test('a built claim index round-trips back to the claimed output', () => {
  ...
  const block = ctx.get(BlockStore).ingest({
    source: AtomSource.Local,
    receivedAt: 0,
    raw: ctx.get(AtomSerializerService).serialize(AtomType.Block, built),
  });
