...
suite('types', function () {
  ...
  suite('BytesType', function () {
    ...
    test('clone', function () {
      ...
      clone = t.clone({ type: 'Buffer', data: Array.from(buf) }, {
        coerceBuffers: true,
      });
      assert.deepEqual(clone, buf);
      assert.throws(function () {
  ...
  suite('UnwrappedUnionType', function () {
    ...
    test('invalid read', function () {
      ...
      assert.throws(function () {
        type.fromBuffer(utils.bufferFrom([4]));
      });
    });
  ...
  suite('WrappedUnionType', function () {
    ...
    test('resolve [string, int] to [long, bytes]', function () {
      ...
      var buf;
      buf = t1.toBuffer({ string: 'hi' });
      assert.deepEqual(t2.fromBuffer(buf, a), {
  ...
  suite('ArrayType', function () {
    ...
    test('resolve string items to bytes items', function () {
      ...
      var obj = ['\x01\x02'];
      var buf = t1.toBuffer(obj);
      assert.deepEqual(t2.fromBuffer(buf, resolver), [
        utils.bufferFrom([1, 2]),
      ]);
    });
  ...
  suite('RecordType', function () {
    ...
    test('isValid hook', function () {
      ...
      try {
        assert(
          !t.isValid({ age: 23, names: ['ann', null] }, { errorHook: hook }),
        );
      } catch (err) {
        hasErr = true;
      }
    ...
    suite('unpacked', function () {
      var slowLongType = builtins.LongType.__with({
        fromBuffer: function (buf) {
          ...
          var n = new DataView(buf.buffer).getInt32(0, true) +
            Math.pow(2, 32) * new DataView(buf.buffer).getInt32(4, true);
          if (neg) {
            invert(buf);
            n = -n - 1;
          }
    ...
    test('within unwrapped union', function () {
      var longType = builtins.LongType.__with({
        ...
        isValid: function (obj) {
          return obj && obj.value instanceof Uint8Array;
        },
        compare: function () {
          throw new Error();
        },
      }, true);
      var t = Type.forSchema(['null', 'long'], {
  ...
  suite('LogicalType', function () {
    ...
    DateType.prototype._fromValue = function (val) {
      return new Date(val);
    };
    ...
    test('recursive', function () {
      ...
      PersonType.prototype._toValue = function (val) {
        return val;
      };
    ...
    test('recursive dereferencing name', function () {
      ...
      BoxType.prototype._fromValue = function (val) {
        return val.unboxed;
      };

      BoxType.prototype._toValue = function (any) {
        return { unboxed: any };
      };
    ...
    test('even integer', function () {
      ...
      assert.throws(function () {
        t.fromBuffer(utils.bufferFrom([2]));
      });
    });
    ...
    test('of records inside wrapped union', function () {
      ...
      PassThroughType.prototype._fromValue = function (val) {
        return val;
      };
  ...
  suite('fromString', function () {
    ...
    test('coerce buffers', function () {
      var t = Type.forSchema({
        ...
        fields: [{
          name: 'id1',
          type: { name: 'Id1', type: 'fixed', size: 2 },
        }],
      });
  ...
  suite('forTypes', function () {
    ...
    test('logical types', function () {
      ...
      function EvenType(schema, opts) {
        LogicalType.call(this, schema, opts);
      }
      inherits(EvenType, LogicalType);
      EvenType.prototype._fromValue = function (val) {
        return 2 * val;
      };
      ...
      function OddType(schema, opts) {
        LogicalType.call(this, schema, opts);
      }
      inherits(OddType, LogicalType);
      OddType.prototype._fromValue = function (val) {
        return 2 * val + 1;
      };
...
function assertUnionsEqual(t1, t2) {
  ...
  var b1 = utils.toMap(t1.types, function (t) {
    return t.branchName;
  });
