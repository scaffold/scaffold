...
suite('utils', function () {
  ...
  test('hasDuplicates', function () {
    ...
    assert(utils.hasDuplicates(['ab', 'cb'], function (s) {
      return s[1];
    }));
  });
  ...
  suite('Tap', function () {
    ...
    suite('bytes', function () {
      testWriterReader({
        elems: [
          utils.bufferFrom('abc'),
          utils.newBuffer(0),
          utils.bufferFrom([1, 5, 255]),
        ],
        reader: function () {
    ...
    suite('pack & unpack longs', function () {
      ...
      test('unpack multiple bytes', function () {
        ...
        t.pos = 0;
        assert.deepEqual(
          new DataView(t.unpackLongBytes().buffer).getInt32(0, true),
          l,
        );
        t.pos = 0;
        l = -3210984;
      ...
      test('roundtrip bytes', function () {
        ...
        roundtrip(
          utils.bufferFrom([0x90, 0x07, 0x19, 0x92, 0x54, 0x74, 0x09, 0x95]),
        );
