...
import { BlockIngestor, serializeBlock } from '../src/core/Ingestor.ts';
import {
  AtomSource,
  AtomType,
  Block,
...
function setup(): Fixture {
  ...
  const serializer = ctx.get(AtomSerializerService);
  let receivedAt = 0;
...
Deno.test('deserialize rejects a signed message that is not a block payload', () => {
  ...
  const raw = ctx.get(AtomSerializerService).serialize(
    AtomType.Block,
