...
- **`wasi_setup` absent → ContractRejection.** RESOLVED. `WasmHostBridge.contractMetadata` now catches `ContractRejection` from the typed env and emits the `(0, 0)` empty-bytes sentinel on the wire; `env.zig`'s `contractMetadata` returns `?[]const u8` (null on `(0, 0)`) and `setup.read` falls through to `defaults()` for the null case. The typed `ContractEnv.contractMetadata` keeps its strict-throws contract for non-WASM callers.
...
- **`/out/debug` routing.** RESOLVED. `WasmHostBridge` now exposes a `debug` flat export wired through all three transports (`InProcessMockTransport`, `JspiTransport`, `AtomicsWorkerTransport`) plus the worker-side `wasmInstance.ts`. `paths.appendDebug` line-buffers and flushes via `env.debug`; `DebugNode.close` flushes the trailing partial line via `autoCloseAll`. Follow-up: `VerifyingEnv` / `GeneratingEnv` don't yet implement the optional `ContractEnv.debug` method, so production `/out/debug` writes are dropped silently. Wire a `ScopedLogger` through `ContractHost` (probably via `ContractHostConfig`) so the default envs route to `ctx.logger('contract').debug(...)`.

### WASI shim — gaps surfaced during Phase E snapshot tests

- **Bump arena vs `.rodata` collision.** RESOLVED. `bump_ptr` started at 1 MiB exactly where wasm-ld loads `.rodata` (also at `__stack_pointer = 1 MiB`). The first `alloc(...)` overwrote the start of `.rodata`, which scrambled string literals like `"/dev/null"` and produced confusing `vfs.NotFound` errors during `populateFdTable`. Fixed by moving `BUMP_START` to 2 MiB (well clear of stack + `.rodata` + `.data` + BSS) and bumping `exe.initial_memory` to 4 MiB in `build.zig`. Long-term cleanup: pass `--export=__heap_base` to wasm-ld and use the linker-provided value instead of the hardcoded 2 MiB. Today's value has 1 MiB of headroom over the largest BSS we can reasonably grow without restructuring `current_state`.
- **`@read` / `@write` accessor markers in `WasmModules.ts`.** RESOLVED. The design doc specified a `parseTargetRef`-recognised `@read` / `@write` suffix on memory targets that should synthesise a cross-memory memcpy closure (the `program_mem.{read,write}_bytes` mechanism the WASI shim depends on for every `fd_read` / `fd_write`). The implementation didn't exist. Added `accessor: 'read' | 'write'` to `TargetRef`, the parser branch in `parseTargetRef`, and `makeAccessorForwarder` in `loadModules`. Memories are resolved lazily at call time so the shim ↔ program function-ring doesn't introduce a topo-sort cycle. Limitations to revisit: (1) the synth closure assumes signature `(target_off: i32, peer_off: i32, len: i32) -> ()` — there's no kind-check at instantiate time, only at call time. (2) The "peer" memory is the source layer's *primary* memory (`memoryByLayerKey.get(srcLayerKey)`); a layer that imports several memories with different roles can't disambiguate.
- **PLAN.md `wasi_clock_realtime` expected bytes were wrong.** Documented in the per-fixture PLAN as `1234 * 10^6 = 0x49949140 LE = 40 91 94 49 ...`. Actual: `1234000000 = 0x498D5880 LE = 80 58 8d 49 ...`. The shim is correct; the PLAN's hex conversion was off. Test (`tests/WasiShim.test.ts`) uses the right bytes; PLAN.md is one of the per-fixture docs and is now drift from reality.
