...
var TAP = new Tap(new Uint8Array(1024));
  ...
  if (schema) {
    ...
    if (name !== undefined) {
      ...
      if (isPrimitive(name)) {
        ...
        throw new Error(
          'cannot rename primitive type: ' + JSON.stringify(name),
        );
      }
      ...
      throw new Error(
        'missing name property in schema: ' + JSON.stringify(schema),
      );
    }
    this.name = name;
...
Type.forSchema = function (schema, opts) {
  ...
  var UnionType = (function (wrapUnions) {
    ...
    switch (wrapUnions) {
      ...
      default:
        throw new Error(
          'invalid wrap unions option: ' + JSON.stringify(wrapUnions),
        );
    }
  })(opts.wrapUnions);
  ...
  if (schema.logicalType && opts.logicalTypes && !LOGICAL_TYPE) {
    ...
    if (DerivedType) {
      ...
      try {
        ...
        console.warn(
          'failed to instantiate logical type for ' + schema.logicalType,
        );
...
Type.forValue = function (val, opts) {
  ...
  if (opts.valueHook) {
    ...
    if (type !== undefined) {
      if (!Type.isType(type)) {
        throw new Error(
          'invalid value hook return value: ' + JSON.stringify(type),
        );
      }
      return type;
    }
...
Type.__reset = function (size) {
  console.debug('resetting type buffer to ' + size);
  TAP.buf = new Uint8Array(size);
};
...
Type.prototype.equals = function (type, opts) {
  var canon = ( // Canonical equality.
    Type.isType(type) &&
    this.fingerprint() === type.fingerprint()
  );
...
Type.prototype.fingerprint = function () {
  if (!this._hash.str) {
    var schemaStr = JSON.stringify(this.schema());
    this._hash.str = utils.getHash(schemaStr);
  }
...
Type.prototype.toBuffer = function (val) {
  ...
  if (TAP.isValid()) {
    buf.set(TAP.buf.subarray(0, TAP.pos));
  } else {
...
BytesType.prototype._check = function (val, flags, hook) {
  var b = val instanceof Uint8Array;
  if (!b && hook) {
    hook(val, this);
  }
  return b;
...
BytesType.prototype.compare = utils.compareBuffers;
...
function EnumType(schema, opts) {
  ...
  this.symbols.forEach(function (symbol, i) {
    ...
    if (this._indices[symbol] !== undefined) {
      throw new Error(
        'duplicate ' + this + ' symbol: ' + JSON.stringify(symbol),
      );
    }
  ...
  if (this.default !== undefined && this._indices[this.default] === undefined) {
    throw new Error(
      'invalid ' + this + ' default: ' + JSON.stringify(this.default),
    );
  }
  this._branchConstructor = this._createBranchConstructor();
  Object.freeze(this);
...
FixedType.prototype._check = function (val, flags, hook) {
  var b = val instanceof Uint8Array && val.length === this.size;
  if (!b && hook) {
    hook(val, this);
  }
  return b;
...
FixedType.prototype._write = function (tap, val) {
  if (!(val instanceof Uint8Array) || val.length !== this.size) {
    throwInvalidError(val, this);
  }
...
FixedType.prototype.compare = utils.compareBuffers;
...
function RecordType(schema, opts) {
  ...
  if (!Array.isArray(schema.fields)) {
    throw new Error(
      'non-array record fields: ' + JSON.stringify(schema.fields),
    );
  }
  if (
...
RecordType.prototype._createWriter = function () {
  ...
  for (i = 0, l = this.fields.length; i < l; i++) {
    ...
    if (field.defaultValue() === undefined) {
      ...
      value = utils.bufferToString(field.type.toBuffer(field.defaultValue()));
...
RecordType.prototype._update = function (resolver, type, opts) {
  ...
  for (i = 0; i < rFields.length; i++) {
    ...
    if (matches.length > 1) {
      throw new Error(
        'ambiguous aliasing for ' + type.name + '.' + field.name + ' (' +
          matches + ')',
      );
    }
    if (!matches.length) {
...
function Field(schema, opts) {
  ...
  if (value !== undefined) {
    ...
    try {
      ...
      var msg = 'incompatible field default ' + JSON.stringify(value) + ' (' +
        err.message + ')';
      if (Type.isType(type, 'union')) {
        msg += ', union defaults must match the first branch\'s type (' +
          JSON.stringify(type.types[0]) + ')';
      }
      throw new Error(msg);
    }
...
function throwInvalidError(val, type) {
  throw new Error(
    'invalid ' + JSON.stringify(type.schema()) + ': ' + JSON.stringify(val),
  );
}
