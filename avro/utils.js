...
function bufferFrom(data, enc) {
  if (enc !== undefined) {
    throw new Error('deprecated property enc');
  }
 ...
 * Compare two buffers.
 *
 * @param a {Buffer} The first one.
 * @param b {Buffer} The second one.
 */
...
function addDeprecatedGetters(obj, props) {
  ...
  var i, l, prop, getter, fn;
  for (i = 0, l = props.length; i < l; i++) {
    prop = props[i];
    getter = 'get' + capitalize(prop);
    fn = createGetter(prop);
    proto[getter] = function () {
      console.warn('use `.' + prop + '` instead of `.' + getter + '()`');
      return fn();
    };
  }
...
Tap.prototype.readFloat = function () {
  ...
  return new DataView(this.buf.buffer).getFloat32(pos, true);
};
...
Tap.prototype.writeFloat = function (f) {
  ...
  new DataView(this.buf.buffer).setFloat32(pos, f, true);
  return this.pos;
};
...
Tap.prototype.readDouble = function () {
  ...
  return new DataView(this.buf.buffer).getFloat64(pos, true);
};
...
Tap.prototype.writeDouble = function (d) {
  ...
  new DataView(this.buf.buffer).setFloat64(pos, d, true);
  return this.pos;
};
...
Tap.prototype.writeBinary = function (s, len) {
  ...
  this.buf.set(stringToBuffer(s), pos);
};
...
Tap.prototype.matchFixed = function (tap, len) {
  return compareBuffers(this.readFixed(len), tap.readFixed(len));
};
...
Tap.prototype.packLongBytes = function (buf) {
  ...
  var parts = [
    new DataView(buf.buffer).getUint32(0, true) & 0xffffff,
    new DataView(buf.buffer).getUint32(3, true) & 0xffffff,
    new DataView(buf.buffer).getUint32(4, true) >>> 16,
  ];
